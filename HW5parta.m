...
for classes = 1:5
    ...
    for iter = 1:itercount
        ...
        for i = 1:gaussiannum
            FGmatrix(:,i) = mvnpdf(TrainsampleDCT_FG,FGmean(i,:),FGstd(:,:,i))*FGweight(i);
            FGmatrix(:,i) = FGmatrix(:,i)./(FGposs*FGweight');

            FGtotal = sum(FGhmatrix(:,i));
            FGweight(i) = FGtotal/size(FGhmatrix,1);
            FGmean(i,:) = sum(repmat(FGmatrix(:,i),1,featuredim).*TrainsampleDCT_FG)'/FGtotal;
    ...
    for iter = 1:itercount
        ...
        for i = 1:gaussiannum
            BGmatrix(:,i) = mvnpdf(TrainsampleDCT_BG,BGmean(i,:),BGstd(:,:,i))*BGweight(i);
            BGmatrix(:,i) = BGmatrix(:,i)./(BGposs*BGweight');

            BGtotal = sum(BGhmatrix(:,i));
            BGweight(i) = BGtotal/size(BGhmatrix,1);
            BGmean(i,:) = sum(repmat(BGmatrix(:,i),1,featuredim).*TrainsampleDCT_BG)'/BGtotal;
...
for classes1 = 1:5
    for classes2 = 1:5
        ...
        for j = 1:totalblock
            ...
            probability2 = 0;
            for k = 1:component
                probability1 = probability1+mvnpdf(imagefeature(j,:),FGtotalmean(k,:,classes1),FGtotalstd(:,:,k,classes1))*FGtotalweight(1,k,classes1);
                probability2 = probability2+mvnpdf(imagefeature(j,:),BGtotalmean(k,:,classes2),BGtotalstd(:,:,k,classes2))*BGtotalweight(1,k,classes2);
            end
        ...
        errorstorer(counter) = sum(abs(graphafter-reshape(hohoho,[1,247*262])))/(247*262);
        counter = counter+1;
    end
end
